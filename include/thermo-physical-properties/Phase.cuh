...
class Phase
{
	private:
		...
		__host__ __device__ __forceinline__ static double _getSigmoid(double x, double origin = 0, double scale = 0)
		{
			return 1 / (1 + exp( - scale * (x - origin)));
		}

		__host__ __device__ __forceinline__ static double _getSigmoidDerivative(double x, double origin = 0, double scale = 0)
		{
